            ...
            httpGet(params) {response ->
                ...
                if (response.status == 200) {
                    ...
                    if (station_id > 0) {
                        ...
                        sendEvent(name: "station_id", value: station_id as Integer, isStateChange: true)
                    } else {
                    	log.error "station_id error: ${station_id}"
                        sendEvent(name: "station_id", value: 0, isStateChange: true)
                    }
                    
                    if (station_name != "") {
                        debugLog("station_name: ${station_name}")
                        sendEvent(name: "station_name", value: station_name as String, isStateChange: true)
                    } else {
                    	log.error "station_name error: ${station_name}"
                        sendEvent(name: "station_name", value: "-", isStateChange: true)
                    }
                    
                    if (latitude > 0) {
                        debugLog("latitude: ${latitude}")
                        sendEvent(name: "latitude", value: latitude as String, isStateChange: true)
                    } else {
                    	log.error "latitude error: ${station_id}"
                        sendEvent(name: "latitude", value: "", isStateChange: true)
                    }
                    
                    if (longitude > 0) {
                        debugLog("longitude: ${longitude}")
                        sendEvent(name: "longitude", value: longitude as String, isStateChange: true)
                    } else {
                    	log.error "longitude error: ${longitude}"
                        sendEvent(name: "longitude", value: "", isStateChange: true)
                    }

                    if (timezone != "") {
                        debugLog("timezone: ${timezone}")
                        sendEvent(name: "timezone", value: timezone as String, isStateChange: true)
                    } else {
                    	log.error "timezone error: ${timezone}"
                        sendEvent(name: "timezone", value: "-", isStateChange: true)
                    }
                    ...
                    if (units_temp != "") {
                        ...
                        sendEvent(name: "units_temp", value: units_temp as String, isStateChange: true)
                    } else {
                        log.error "units_temp error: ${units_temp}"
                        sendEvent(name: "units_temp", value: "", isStateChange: true)
                    }

                    if (units_wind != "") {
                        debugLog("units_wind: ${units_wind}")
                        sendEvent(name: "units_wind", value: units_wind as String, isStateChange: true)
                    } else {
                        log.error "units_wind error: ${units_wind}"
                        sendEvent(name: "units_wind", value: "", isStateChange: true)
                    }

                    if (units_precip != "") {
                        debugLog("units_precip: ${units_precip}")
                        sendEvent(name: "units_precip", value: units_precip as String, isStateChange: true)
                    } else {
                        log.error "units_precip error: ${units_precip}"
                        sendEvent(name: "units_precip", value: "", isStateChange: true)
                    }

                    if (units_pressure != "") {
                        debugLog("units_pressure: ${units_pressure}")
                        sendEvent(name: "units_pressure", value: units_pressure as String, isStateChange: true)
                    } else {
                        log.error "units_temp error: ${units_pressure}"
                        sendEvent(name: "units_pressure", value: "", isStateChange: true)
                    }

                    if (units_distance != "") {
                        debugLog("units_distance: ${units_distance}")
                        sendEvent(name: "units_distance", value: units_distance as String, isStateChange: true)
                    } else {
                        log.error "units_distance error: ${units_distance}"
                        sendEvent(name: "units_distance", value: "", isStateChange: true)
                    }

                    if (units_direction != "") {
                        debugLog("units_direction: ${units_direction}")
                        sendEvent(name: "units_direction", value: units_direction as String, isStateChange: true)
                    } else {
                        log.error "units_direction error: ${units_direction}"
                        sendEvent(name: "units_direction", value: "", isStateChange: true)
                    }

                    if (units_other != "") {
                        debugLog("units_other: ${units_other}")
                        sendEvent(name: "units_other", value: units_other as String, isStateChange: true)
                    } else {
                        log.error "units_other error: ${units_other}"
                        sendEvent(name: "units_other", value: "", isStateChange: true)
                    }
                    ...
                    if (timestamp != "") {
                        ...
                        sendEvent(name: "timestamp", value: convertDate, isStateChange: true)
                    } else {
                        log.error "timestamp error: ${timestamp}"
                        sendEvent(name: "timestamp", value: 0, isStateChange: true)
                    }

                    if (air_temperature != "") {
                        debugLog("air_temperature: ${air_temperature}°${units_temp}")
                        sendEvent(name: "temperature", value: air_temperature, unit: units_temp, isStateChange: true)
                    } else {
                        log.error "air_temperature error: ${air_temperature}"
                        sendEvent(name: "temperature", value: -100, isStateChange: true)
                    }

                    if (relative_humidity != "") {
                        debugLog("relative_humidity: ${relative_humidity}°${units_temp}")
                        sendEvent(name: "humidity", value: relative_humidity, unit: '%', isStateChange: true)
                    } else {
                        log.error "relative_humidity error: ${relative_humidity}"
                        sendEvent(name: "humidity", value: -100, isStateChange: true)
                    }
                    
                    if (dew_point != "") {
                        debugLog("dew_point: ${dew_point}°${units_temp}")
                        sendEvent(name: "dew_point", value: dew_point, unit: units_temp, isStateChange: true)
                    } else {
                        log.error "dew_point error: ${dew_point}"
                        sendEvent(name: "dew_point", value: -100, isStateChange: true)
                    }
                    ...
                    if (barometric_pressure != "") {
                        ...
                        sendEvent(name: "barometric_pressure", value: barometric_pressure, unit: units_pressure, isStateChange: true)
                    } else {
                        log.error "barometric_pressure error: ${barometric_pressure}"
                        sendEvent(name: "barometric_pressure", value: -100, isStateChange: true)
                    }
                    ...
                    if (sea_level_pressure != "") {
                        ...
                        sendEvent(name: "sea_level_pressure", value: sea_level_pressure, unit: units_pressure, isStateChange: true)
                    } else {
                    ...
                    if (precip_accum_local_day != "") {
                        ...
                        sendEvent(name: "precip_accum_local_day", value: precip_accum_local_day, unit: units_precip, isStateChange: true)
                    } else {
                        log.error "precip_accum_local_day error: ${precip_accum_local_day}"
                        sendEvent(name: "precip_accum_local_day", value: 0, isStateChange: true)
                    }
                    ...
                    if (precip_accum_local_yesterday != "") {
                        ...
                        sendEvent(name: "precip_accum_local_yesterday", value: precip_accum_local_yesterday, unit: units_precip, isStateChange: true)
                    } else {
                    ...
                    if (wind_avg != "") {
                        ...
                        sendEvent(name: "wind_avg", value: wind_avg, unit: units_wind, isStateChange: true)
                    } else {
                        log.error "wind_avg error: ${wind_avg}"
                        sendEvent(name: "wind_avg", value: 0, isStateChange: true)
                    }
                    
                    if (wind_direction != "") {
                        debugLog("wind_direction: ${wind_direction}°")
                        sendEvent(name: "wind_direction", value: wind_direction, unit: "°", isStateChange: true)
                    } else {
                        log.error "wind_avg error: ${wind_avg}"
                        sendEvent(name: "wind_avg", value: 0, isStateChange: true)
                    }
                    ...
                    if (wind_gust != "") {
                        ...
                        sendEvent(name: "wind_gust", value: wind_gust, unit: units_wind, isStateChange: true)
                    } else {
                        log.error "wind_gust error: ${wind_gust}"
                        sendEvent(name: "wind_gust", value: 0, isStateChange: true)
                    }
                    
                    if (solar_radiation != "") {
                        debugLog("solar_radiation: ${solar_radiation}")
                        sendEvent(name: "solar_radiation", value: solar_radiation, unit: "W/㎡", isStateChange: true)
                    } else {
                        log.error "solar_radiation error: ${solar_radiation}"
                        sendEvent(name: "solar_radiation", value: 0, isStateChange: true)
                    }
                    
                    if (uv != "") {
                        debugLog("uv: ${uv}")
                        sendEvent(name: "ultravioletIndex", value: uv, isStateChange: true)
                    } else {
                        log.error "uv error: ${uv}"
                        sendEvent(name: "ultravioletIndex", value: 0, isStateChange: true)
                    }
                    
                    if (brightness != "") {
                        debugLog("brightness: ${brightness}")
                        sendEvent(name: "illuminance", value: brightness, unit: "lux", isStateChange: true)
                    } else {
                        log.error "brightness error: ${brightness}"
                        sendEvent(name: "illuminance", value: 0, isStateChange: true)
                    }
                    ...
                    if (lightning_strike_last_epoch != "") {
                        ...
                        sendEvent(name: "lightning_strike_last_epoch", value: convertDate, isStateChange: true)
                    } else {
                        log.error "lightning_strike_last_epoch error: ${lightning_strike_last_epoch}"
                        sendEvent(name: "lightning_strike_last_epoch", value: 0, isStateChange: true)
                    }
                    ...
                    if (lightning_strike_last_distance != "") {
                        ...
                        sendEvent(name: "lightning_strike_last_distance", value: lightning_strike_last_distance, unit: units_distance, isStateChange: true)
                    } else {
                        log.error "lightning_strike_last_distance error: ${lightning_strike_last_distance}"
                        sendEvent(name: "lightning_strike_last_distance", value: 0, isStateChange: true)
                    }
                    
                    if (lightning_strike_count_last_3hr != "") {
                        debugLog("lightning_strike_count_last_3hr: ${lightning_strike_count_last_3hr}")
                        sendEvent(name: "lightning_strike_count_last_3hr", value: lightning_strike_count_last_3hr, isStateChange: true)
                    } else {
                        log.error "lightning_strike_count_last_3hr error: ${lightning_strike_count_last_3hr}"
                        sendEvent(name: "lightning_strike_count_last_3hr", value: 0, isStateChange: true)
                    }
          		}
