...
metadata {
	definition (name: "SmartWeather Station For Korea", namespace: "WooBooung", author: "Booung", ocfResourceType: "x.com.st.airqualitylevel") {
		...
		// Air Korea infos
        attribute "airQualityStatus", "number"
	...
	tiles {
        ...
        valueTile("airquality_infos", "", width: 6, height: 1, decoration: "flat") {
        ...
        valueTile("wunderground_infos", "", width: 6, height: 1, decoration: "flat") {
        ...
        valueTile("rise_label", "", decoration: "flat") {
			state "default", label:'일출'
		}

		valueTile("rise_value", "device.localSunrise",decoration: "flat") {
			state "default", label:'${currentValue}'
		}

		valueTile("set_label", "", decoration: "flat") {
			state "default", label:'일몰'
		}

		valueTile("set_value", "device.localSunset", decoration: "flat") {
			state "default", label:'${currentValue}'
		}
		
        valueTile("refresh_label", "", decoration: "flat") {
			state "default", label:'Refresh', action: "linkStationSearch"
		}
       
        standardTile("refresh_value", "device.weather", decoration: "flat") {
		...
		details(["airQuality",
                ...
                "wunderground_infos",
                "weatherIcon_value", "temperature_label", "humidity_label", "ultravioletIndex_label", "illuminance_label", "city_label", 
                "weather_value", "temperature_value", "humidity_value", "ultravioletIndex_value", "illuminance_value", "city_value", 
                "feelsLike_label", "wind_label", "percentPrecip_label", "rise_label", "set_label", "refresh_label",
                "feelsLike_value", "wind_value", "percentPrecip_value", "rise_value", "set_value", "refresh_value"])}
}
            ...
            httpGet(params) {resp ->
                ...
                if (resp.status == 200) {
                    ...
                    if( resp.data.list[0].o3Value != "-" )  {
                        ...
                        Float f = Float.parseFloat(resp.data.list[0].o3Value)
                        Float display_value = f.round(3)
                    ...
                    if( resp.data.list[0].no2Value != "-" ) {
                        ...
                        Float f = Float.parseFloat(resp.data.list[0].no2Value)
                        Float display_value = f.round(3)
                    ...
                    if( resp.data.list[0].so2Value != "-" ) {
                        ...
                        Float f = Float.parseFloat(resp.data.list[0].so2Value)
                        Float display_value = f.round(3)
                    ...
                    if( resp.data.list[0].coValue != "-" ) {
                        ...
                        Float f = Float.parseFloat(resp.data.list[0].coValue)
                        Float display_value = f.round(3)
                        sendEvent(name: "carbonMonoxide", value: display_value, unit: "ppm", isStateChange: true)
                        sendEvent(name: "co_value", value: display_value as String, unit: "ppm", isStateChange: true)
                    } else {
                    	sendEvent(name: "carbonMonoxide", value: "--", unit: "ppm", isStateChange: true)
                    }
